...
public class ImageInputOutput {
	...
	public Pixmap loadImage(String filePath) {
		...
		System.arraycopy(fileBytes, 0, _fileHeader, 0, startPoint);
		...
		for(int i = startPoint; i < fileIntData.length - 3; i += 3) {
			r = fileIntData[i+2]; g = fileIntData[i+1]; b = fileIntData[i];
			pixels.setColor((float)(r/256.0),(float)(g/256.0),(float)(b/256.0), 1);
